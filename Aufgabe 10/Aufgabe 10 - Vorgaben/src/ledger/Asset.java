...
public class Asset extends Account {
	...
	public Asset(String name, int initialValue) {
	...
	public void credit(int value) throws Exception {
		if (!isOpen()) {
			throw new AccountClosedException("Cannot credit closed asset account: " + getName());
		}
		if (value < 0) {
	...
	public void debit(int value) throws Exception {
		if (!isOpen()) {
			throw new AccountClosedException("Cannot debit closed asset account: " + getName());
		}
		if (value < 0) {
	...
	public boolean commit(Accountant accountant) throws Exception {
		...
		if (value + delta < 0) {
			throw new AmountInsufficientException("Insufficient balance in asset account: " + getName());
		}
